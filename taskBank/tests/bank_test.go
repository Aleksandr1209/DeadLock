...
func TestTransferDeadlock(t *testing.T) {
	...
	go func() {
		defer wg.Done()
		bank.TransferDeadlock(0, 1, 100)
	}()

	go func() {
		defer wg.Done()
		bank.TransferDeadlock(1, 0, 50)
	}()
...
func TestTransferCorrect(t *testing.T) {
	...
	go func() {
		defer wg.Done()
		bank.TransferCorrect(0, 1, 100)
	}()

	go func() {
		defer wg.Done()
		bank.TransferCorrect(1, 0, 50)
	}()
...
func TestConcurrentTransfers(t *testing.T) {
	bank := solution.Bank{}
	for i := 0; i < 100; i++ {
		bank.Accounts = append(bank.Accounts, &solution.Account{ID: i, Balance: 1000})
	}
